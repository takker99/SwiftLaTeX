...
Module.compileLaTeX = async () => {
  ...
  let status = await _compileLaTeX();
  if (status === 0) {
    let pdfArrayBuffer = null;
    await _compileBibtex();
...
const kpse_find_file_impl = async (nameptr, format, _mustexist) => {
 ...
 */
// deno-lint-ignore no-unused-vars
const fontconfig_search_font_impl = async (fontnamePtr, varStringPtr) => {
